    ...
    on player chats bukkit_priority:LOWEST:
      ...
      - if <yaml[chat_config].read[channels.<[channel]>.global]>:
        ...
        - if <yaml[chat_config].read[channels.<[channel]>.integrations.Discord.active]>:
          - bungeerun relay chat_send_message def:<list_single[<[Channel]>].include[<player.display_name.strip_color>].include[<context.message>].include[<bungee.server>].include[<player.name>]>
